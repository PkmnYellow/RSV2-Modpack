...
REM --Installation for THC, copying over a the .ini and renaming it.--
:installmod_u
echo Installing Terrorist Hunt Control Mod...
copy /Y "KellerGame\Config\PCKellerInput (Original).ini" %var%\KellerGame\Config"
del /q %var%\KellerGame\Config\PCKellerInput.ini"
cd /D %var%\KellerGame\Config"
echo Reverting name back to default...
ren "PCKellerInput (Original).ini" "PCKellerInput.ini"
cd /D %main%
...
) else ( echo Terrorist Hunt Control [Original] >> installedmods.txt 
...
:modcheck2
set results5=
for /f "tokens=1 delims=" %%f in ('findstr /c:"Terrorist Hunt Control [Original]" installedmods.txt') do set results5=%%f
