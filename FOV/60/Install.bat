...
) else ( echo FOV 60 >> installedmods.txt 
echo Installedmods.txt created...
goto :end )

REM --Checks if "FOV 60" is logged into installedmods.txt.--
:modcheck2
set results9=
for /f "tokens=1 delims=" %%e in ('findstr /i "FOV 60" installedmods.txt') do set results9=%%e

if defined results9 ( goto :end
) else ( echo FOV 60 >> installedmods.txt 
echo FOV 60 added to the mod list!
goto :end )
